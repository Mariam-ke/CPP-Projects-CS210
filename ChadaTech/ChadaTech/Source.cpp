...
int main()
{
	...
	// start the default time to display at midnight for both of the clocks
	displayClock(0, 0, 0);
	...
	cout << "Program exti, Goodbye! " << endl;
	return 0;
}
...
int displayClock(int addHour, int addMinute, int addSecond)
{
	...
	// declare atatuc variables for the 12 hour Clock which is display first
	...
	// If, else statements to switch from AM to PM
	// check if the hours in the first clock is past 12
	if (firstClockHours > 12)
	{

		// check if AM or PM then change it to afternoon to display as PM
		if (dayOrNight == morning)
			dayOrNight = afternoon;

		// when AM or PM is already afternoon change to display AM
		else
			dayOrNight = morning;

		// set the time back to 0:00:00 for the 12 hour clock format
		firstClockHours = 0;
	}

	// check the minutes in the first clock if greater than 59 minutes
	...
	// check if the seconds in the fist clock if greater than 59 minutes
	...
	// check if the seconds in the 24 hour clock if greater than 59
