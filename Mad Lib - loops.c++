...
string 
is quit and disregards the integer input that follows.
...
int main() {
   ...
   int number; //variable for number of items
   
   cin >> name >> number; // user input for name and number if items
   
   while(number != 0) {
      cout << "Eating " << number << " " << name << " a day keeps you happy and healthy." << endl;
      cin >> name >> number;  //ask for user input again
   }
